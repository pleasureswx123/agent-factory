...
1. 在素材资产库中删除素材，只表示从当前素材资产库移除，不删除产生该素材的历史会话消息。
2. 如果历史会话消息仍保留，用户可以在消息中再次点击“保存为素材”，把对应产物重新加入素材资产库。
3. 删除会话时，已保存素材不随会话删除，只断开来源会话与消息引用。
4. 删除 Agent 时，Agent 本身软删除，相关会话上下文删除；已保存素材继续保留，并标记为“来源 Agent 已删除”。
...
7. 用户可以在 Agent 管理入口中删除不需要的 Agent。删除前需要二次确认，并提示相关会话会删除、已保存素材会保留。
...
5. MVP 默认删除 Agent 时不物理删除素材资产，而是软删除 Agent、删除该 Agent 的会话上下文，并将相关素材标记为“来源 Agent 已删除”。
6. 删除会话不应删除素材资产库中的素材，只清除素材上的来源会话和来源消息信息。
7. 删除素材不应删除历史会话消息；如果用户后悔删除素材，可以从保留的会话消息中再次点击“保存为素材”。
